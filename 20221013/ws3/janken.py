...
if len(sys.argv)==2:
    ...
    print('使用法: python janken.py 学習済ファイル.pkl')
    sys.exit()
...
class Application(tk.Frame):
    ...
    def janken_loop(self):
        ...
        while jankenLoop == True:
            ...
            message_text = 'じゃんけん'
            self.message_canvas.delete('all')
            self.message_canvas.create_text(200*3, 15*3, text=message_text)
            # 「じゃんけんぽん」という音声を再生
            ...
            # コンピュータが決めた手
            # comp_choice = np.random.randint(0, 3)
            comp_choice = (jpredict[0] + 1)%3
